...
// NewIni Allocate a new empty ini and return it

// SetProperty Set a key-value pair under a certain ini file section
...
func EqualInis(ini1 *IniFile, ini2 *IniFile) bool {
	...
	for section, ini1Entries := range *ini1 {
		ini2Entries, ok := (*ini2)[section]
		if !ok {
			return false
		}
		if len(ini1Entries) != len(ini2Entries) {
			return false
		}

		for key := range ini1Entries {
			if ini1Entries[key] != ini2Entries[key] {
				return false
			}
		}
	}
...
// MergeInis Merge two ini, into one.
...
func MergeInis(ini1 *IniFile, ini2 *IniFile) *IniFile {
	...
	for section, sectionMap := range *ini2 {
